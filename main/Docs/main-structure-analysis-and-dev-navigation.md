...
- `Docs/`: 项目级文档与规范
- 根配置文件: `.env*`、`llm_config.json`、`llm_config.template.json`、`llm_config_schema.json`
...
### 4.1 LLM 配置文件族
仅保留四份权威文件（其余历史副本已在 v5.0 清理中删除）：

- `main/llm_config.json`：运行时主配置（拷贝模板后修改）
- `main/llm_config.template.json`：模板
- `main/llm_config_schema.json`：JSON Schema（Draft 2020-12）
- `docs/sdk/15-llm-config.md`：使用与字段说明
...
### 场景 4: 接入新模型或调整模型配置
1. 拷贝 `main/llm_config.template.json` 为 `main/llm_config.json` 后修改
2. 校验字段与 `main/llm_config_schema.json` 一致（详见 `docs/sdk/15-llm-config.md`）
3. 引擎调用链核对:
   - `pyagentforge/client.py`
...
1. `main/agentforge-engine/pyagentforge/plugins/integration/perception/PLUGIN.py`（去重、插件生命周期）
2. `main/agentforge-engine/pyagentforge/plugins/integration/perception/perception.py`（决策逻辑）
3. `main/agentforge-engine/pyagentforge/plugins/integration/perception/strategy.py`（策略控制）
4. 回归测试: `main/agentforge-engine/pyagentforge/plugins/integration/perception/tests/*.py`
...
- perception:
  - `cd main/agentforge-engine && py -m pytest pyagentforge/plugins/integration/perception/tests/ -v`
