...
# ``ApiType`` 保留为类型别名以兼容旧代码（例如类型注解 / isinstance 判断）。
# 关键变化：运行时不再做 Literal 枚举校验——真正的有效性由
# ``pyagentforge.protocols.PROTOCOL_ADAPTERS`` 在请求时动态判断，这样
# 第三方插件只需通过 entry_points / register_protocol_adapter 注册新
# ``api_type``，无需修改 kernel 源码。
ApiType = str
...
class ModelConfig(BaseModel):
    ...
    api_type: str = Field(
        ...,
        description=(
            "协议格式类型。运行时由 PROTOCOL_ADAPTERS 校验，"
            "任何通过 entry_points/register_protocol_adapter 注册的 api_type 均可使用。"
        ),
    )
...
class ModelRegistry:
    ...
    def list_models(self) -> dict[str, ModelConfig]:
        """Backward-compatible model listing API."""
        return dict(self._get_effective_models())
