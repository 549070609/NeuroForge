...
支持从 JSON 配置文件加载模型和 Provider 配置
"""
...
class ModelRegistry:
    ...
    def __init__(self, load_from_config: bool = True) -> None:
        ...
        Args:
            load_from_config: 是否从 JSON 配置文件加载模型
        """
    ...
    def _load_from_json_config(self) -> None:
        ...
        try:
            ...
            for model_id, model_cfg in config.models.items():
                # 转换 provider 字符串为 ProviderType
                provider_str = model_cfg.provider.lower()
                try:
                ...
                if not api_key:
                    api_key = manager.get_api_key(model_cfg.provider, model_id)

                # 获取 Base URL
                base_url = model_cfg.base_url
                if not base_url:
                    base_url = manager.get_base_url(model_cfg.provider, model_id)

                # 构建 api_key_env
                api_key_env = model_cfg.api_key_env or ""
                if not api_key_env:
                    provider_config = manager.get_provider_config(model_cfg.provider)
                ...
                # 覆盖或添加模型
                self._models[model_id] = mc

            logger.info(
                "Loaded models from JSON config",
                extra_data={"count": len(config.models)},
            )

        except Exception as e:
            logger.warning(
                f"Failed to load from JSON config: {e}, using builtin models only",
            )
